...
if [ -f ${PROJECT_PATH}/done/vep/${CONTIG}_VEP.vcf.gz.done ] || [ -f ${PROJECT_PATH}/done/ann/${CONTIG}_ann.vcf.gz.done ]; then
...
if [ -f ${PROJECT_PATH}/done/vep/${CONTIG}_VEP.vcf.gz.tbi.done ] || [ -f ${PROJECT_PATH}/done/ann/${CONTIG}_ann.vcf.gz.tbi.done ]; then
	...
	scontrol update jobid=${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID} jobname=VEPindex_${PROJECT}_${CONTIG}
	STARTTIME=$(date +%s)
	if [ ${#sourcetagarray[@]} -eq 0 ]; then
		output=${PROJECT_PATH}/ann/${CONTIG}_ann.vcf.gz
	else
