...
echo "AnnoSnpEff on $(date) on $(hostname)"
echo "$0 $*"
...
if  [ -f "${PROJECT_PATH}/done/snpeff/${CONTIG}_SnpEff.vcf.gz.done" ] || \
...
then
	...
	then
		...
		scontrol update \
			...
			jobname=SnpEff_${PROJECT}_${CONTIG}
...
##snpeff annotation
	...
	cmd="srun \
$(which java) -Xmx${SLURM_MEM_PER_NODE}m \
...
${snpeffdataversion} \
${input} | \
$(which bcftools) view -Oz -o ${output}"
	echo $cmd
	eval $cmd || exit $?
